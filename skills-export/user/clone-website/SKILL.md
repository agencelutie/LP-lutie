...
**Complex section** (3+ distinct sub-components): Break it up. One agent per sub-component, plus one agent for the section wrapper that imports them.
...
**Don't wait.** As soon as you've dispatched the builder(s) for one section, move to extracting the next section.
...
- Implement the page-level layout from your topology doc
...
1. Open the original site and your clone side-by-side
...
- **Don't extract only the default state.** Click every tab, capture every state.
